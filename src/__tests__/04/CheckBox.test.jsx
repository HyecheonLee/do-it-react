...
describe('<CheckBox>', () => {
	...
	it('displays errorMessage', () => {
		...
		const errorHtml = shallow(<CheckBox name={"name"}
		                                    errorMessage={errorMessage}>테스트</CheckBox>)
		.dive().find('span')
		.html();
	...
	it('calls back onChange on clicked', () => {
		...
		const input = shallow(<CheckBox name={"required"}
		                                onChange={changeStub}>테스트</CheckBox>)
		.dive()
		.find('input');
